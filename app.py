...
MODEL_PATH = "./saved_model/decision_tree.joblib"
...
def predict_disease(symptoms_selected):
    ...
    input_data = np.array(list(symptoms_dict.values())).reshape(1, -1)
    prediction = model.predict(input_data)
    return prediction[0]

# Streamlit UI
st.set_page_config(page_title="Disease Predictor", layout="centered", page_icon="🩺")

st.title("🩺 Disease Prediction from Symptoms")
st.markdown(
    """
    This application predicts the **most likely disease** based on the symptoms you select.
    > ⚠️ This tool is for educational purposes only and **not** for medical use.
    """
)
